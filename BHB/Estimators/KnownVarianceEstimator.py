...
class KnownVarianceEstimator(BaseEstimator):
    # Factors in estimates of the cluster variance into the posterior distribution of cluster means
    def __init__(self, environment, est_priors = None):
        ...
        for b in range(self.nbClusters):
            self.cluster_beliefs.append(self.ClusterBelief(self, b, self.hyperprior_means[b], self.hyperprior_variances[b]))
    ...
    class ClusterBelief:
        ...
        def __init__(self, estimator, cluster_index, hyperprior_mean, hyperprior_variance):
            ...
            self.hyperprior_mean = self.estimator.hyperprior_means[cluster_index]
            self.hyperprior_variance = self.estimator.hyperprior_variances[cluster_index]
            self.pulls = 0
            self.history = {
                "Time": [],
                "ArmBelief": [],
            ...
            for b in range(self.nbArms):
                self.arm_beliefs.append(KnownVarianceEstimator.ArmBelief(self, b))
    ...
    class ArmBelief:
        ...
        def __init__(self, cluster, arm_index):
            ...
            self.cluster_var = cluster.cluster_var
        ...
        def compute_belief_params(self, theta_i):

            nb_obs = len(self.observations)
            if nb_obs > 0:  # If we have no observations,
                s_ij = self.reward_variance / nb_obs
                tau_ij = 1 / s_ij + 1 / self.cluster.cluster_var
                u_ij = 1/tau_ij * (theta_i/ self.cluster_var+self.sample_mean/s_ij)
            else:
                tau_ij = 1 / self.cluster.cluster_var
                u_ij = theta_i
