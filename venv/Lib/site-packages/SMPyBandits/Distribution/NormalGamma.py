...
class NormalGamma(Distribution):
    ...
    def __init__(self, *args):
        ...
        if isinstance(args, tuple):
            args = args[0]
        # Normal Parameters
        self._omega = float(args[0])  # initial value
        self.omega = float(args[0])  #: Parameter :math:`\omega` of the posterior
        self._zeta = float(args[1])  # initial value
        self.zeta = float(args[1])  #: The parameter :math:`\sigma` of the posterior

        # Gamma Parameters
        self._alpha = float(args[2])
        self.alpha = float(args[2])
        self._beta = float(args[3])
        self.beta = float(args[3])
