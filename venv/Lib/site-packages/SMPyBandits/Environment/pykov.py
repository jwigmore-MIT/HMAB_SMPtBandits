...
class Chain(Matrix):
    ...
    def absorbing_time(self, transient_set):
        ...
        Return the ``Vector tau_i``, the ``tau_i[i]`` is the mean number of steps needed
        ...
        .. note::
           ...
           calculate ``p * tau_i`` in order to weigh the mean times according the
           initial conditions.
    ...
    def absorbing_tour(self, p, transient_set=None):
        ...
        .. note::
           ``v.sum()`` is equal to ``p * tau_i`` (see :meth:`absorbing_time` method).
