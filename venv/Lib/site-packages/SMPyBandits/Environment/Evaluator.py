...
try:
    ...
    from .HMABenv import HMABenv
    from .Result import Result
    from .memory_consumption import getCurrentMemory, sizeof_fmt
    from ..Distribution.NormalGamma import NormalGamma
except ImportError:
    # Local imports, libraries
    from usejoblib import USE_JOBLIB, Parallel, delayed
    from usetqdm import USE_TQDM, tqdm
...
plot_lowerbound = False  #: Default is to plot the lower-bound
...
class Evaluator(object):
    ...
    def __init__(self, configuration,
        ...
        self.final_policy = np.empty((self.envs.__len__(),self.nbPolicies)).tolist()#[[]]
        self.final_result = np.empty((self.envs.__len__(), self.nbPolicies)).tolist()#[[]]
    ...
    def __initEnvironments__(self):
        ...
        for configuration_arms in self.cfg['environment']:
            ...
            elif isinstance(configuration_arms, dict) and "isHMAB" in configuration_arms:
                if not configuration_arms["isHMAB"]:
                    print(f""" 'isHMAB' in configuration_arms but is equal to {configuration_arms["isHMAB"]} """)
                    break # If isMAB is False
                new_mab_problem = HMABenv(configuration_arms["envConfig"])


            if new_mab_problem is None:
                new_mab_problem = MAB(configuration_arms)
            self.envs.append(new_mab_problem)
            if configuration_arms["compareMAB"]:
                comp_mab_problem = new_mab_problem.createMAB()
                self.envs.append(comp_mab_problem)

    def __initPolicies__(self, env):
        """ Create or initialize policies."""
        for policyId, policy in enumerate(self.cfg['policies']):
            if policy["isHMAB"]:
                self.policies.append(policy['archtype'](*policy["params"]))
                pass
            else:
    ...
    def startOneEnv(self, envId, env):
        ...
        def store(r, p, policyId, repeatId, t):
            """ Store the result of the #repeatId experiment, for the #policyId policy."""
            self.final_policy[envId][policyId] = p
            self.final_result[envId][policyId] = r
            self.rewards[policyId, envId, :t] += r.rewards
            ...
            if hasattr(self, 'minCumRewards'):
                self.minCumRewards[policyId, envId, :t] = np.minimum(self.minCumRewards[policyId, envId, :], np.cumsum(r.rewards)) if repeatId > 1 else np.cumsum(r.rewards)
            if hasattr(self, 'maxCumRewards'):
                self.maxCumRewards[policyId, envId, :t] = np.maximum(self.maxCumRewards[policyId, envId, :], np.cumsum(r.rewards)) if repeatId > 1 else np.cumsum(r.rewards)
            self.bestArmPulls[envId][policyId, :t] += np.cumsum(np.in1d(r.choices, np.array(r.indexes_bestarm)))  #Does not work
            self.pulls[envId][policyId, :] += r.pulls  # Works
        ...
        for policyId, policy in enumerate(self.policies):
            ...
            if not (env.isHMAB == policy.isHMAB):
                print(f"Environment {envId} is HMAB = {env.isHMAB}")
                print(f"Policy {policyId} is HMAB = {policy.isHMAB}")
            ...
            if self.useJoblib:
                ...
                repeatIdout = 0
                for r,p in Parallel(n_jobs=self.cfg['n_jobs'], pre_dispatch='3*n_jobs', verbose=self.cfg['verbosity'])(
                    ...
                    store(r, p, policyId, repeatIdout)
                    repeatIdout += 1
            else:
                for repeatId in tqdm(range(self.repetitions), desc="Repeat"):
                    r,p, t = delayed_HMAB_play(env, policy, self.horizon, random_shuffle=self.random_shuffle, random_invert=self.random_invert, nb_break_points=self.nb_break_points, allrewards=allrewards, repeatId=repeatId, useJoblib=self.useJoblib)
                    store(r,p, policyId, repeatId, t)
    ...
    # --- Get data
    def getBestArms(self, envId, policyId, verbose = True, k =1):
        # Actual best arm from environment
        env = self.envs[envId]
        pol = self.final_policy[envId][policyId]
        if isinstance(pol, float):
            return
        if env.isHMAB and env.isHMAB:
            envBestArms = self.envs[envId].bestArms[0:k]
            envBestArmVals = self.envs[envId].means[envBestArms]
            polBestArms = self.final_policy[envId][policyId].bestArms[:k]
            polBestArmVals = self.final_policy[envId][policyId].mus_sorted[:k]
        if not env.isHMAB and not pol.isHMAB:
            envBestArm = env.bestArm
            envBestArmVal = env.maxArm
            polAvgRewards = self.getAvgArmReward(envId, policyId)
            polBestArmVal = np.max(polAvgRewards)
            polBestArm = np.argmax(polAvgRewards)
        else:
            pass
        return (envBestArms, envBestArmVals, polBestArms, polBestArmVals)
        #pass
        if verbose:
            print("*" * 20)
            print(f"Environment ({envId}) best arm =  {envBestArm} with value {envBestArmVal}")
            print(f"Policy ({policyId}) estimated best arm =  {polBestArm} with estimated value {polBestArmVal}")

    def getAvgArmReward(self, envId, policyId):
        pol = self.final_policy[envId][policyId]
        polAvgRewards = pol.rewards/pol.pulls
        return polAvgRewards
    ...
    def getBestArmPulls(self, policyId, envId=0):
        """Extract frequency of best arm pulls as a function of t."""
    ...
    def getMaxRewards(self, envId=0):
        """Extract max mean rewards over all policies for a specific ."""
    ...
    def getAverageRewards(self, policyId, envId=0):
        """Extract moving average of rewards(not `rewards` but `cumsum(rewards)/cumsum(1)`."""
        return np.cumsum(self.getRewards(policyId, envId)) / self._times # Array of average rewards for each t
    ...
    # --- Plotting methods
    def plotChoices(self, envId, polId):
        choices = self.final_result[envId][polId].choices
        fig, ax = plt.subplots()
        ax.scatter(range(choices.shape[0]),choices)
        ax.set_title("Bin Selection vs Time")
        ax.set_xlabel("t")
        ax.set_ylabel("Bin choice")
        fig.show()
        return 0
    ...
    def plotRegrets(self, envIds=0,
        ...
        for envId in envIds:
            for i, policy in enumerate(self.policies):
                if not (policy.isHMAB == self.envs[envId].isHMAB):
                    continue
                if meanReward:
            ...
            if not meanReward:
                if semilogy or loglog:
                    ymin = max(0, ymin)
                plt.ylim(ymin, max(plt.ylim()[1],Y.max()))
    ...
    def plotArmPulls(self, envId, policyId):
        pulls = self.final_policy[envId][policyId].pulls
        (envBestArm, envBestArmVal, polBestArm, polBestArmVal) = self.getBestArms(envId, policyId, verbose= False, k=1)
        x = np.array((range(0,pulls.shape[0])))
        fig, ax = plt.subplots()
        colors = []
        for ii in x:
            if ii == envBestArm and ii == polBestArm:
                colors.append('green')
            elif ii == envBestArm:
                ...
                colors.append('blue')
        bar = ax.bar(x, pulls, color = colors)
        #bar[envBestArm].set_color("g")
        if envBestArm == polBestArm:
            bar[envBestArm].set_label("Env. & Policy Best Arm")
        else:
            bar[envBestArm].set_label("Env. (True) Best Arm")
            bar[polBestArm].set_label("Policy (Est) Best Arm")
        ax.set_ylabel("Pulls")
        ax.set_xlabel("Arm")
        plt.xticks(x)
        ax.set_title(f'Total Arm Pulls for EnvId ({envId}) & Policy ({policyId}) ')
        ax.legend()



        plt.show() # Plot histogram of arm pulls
    ...
    def printBandEstStats(self, envId = 0, policyId = 0):
        print("*"*20)
        print(f"Printing True Band Distribution Parameters and Estimates")
        print("-" * 20)
        for i in range(self.envs[envId].nbBands):
            TrueBand = self.envs[envId].Z[i]
            policy = self.final_policy[envId][policyId]
            EstBand = policy.posteriorBand[i]
            print(f"Band {i}")
            print(f"({i})| TRUE Distribution = {TrueBand.bandDistribution} with parameters (loc = {TrueBand.loc},scale = {TrueBand.scale})")
            print(f"({i})| ESTIMATED Distribution = {EstBand.printLikelihood()}")
            print(f"({i})| Number of samples = {np.sum(policy.pullsBB[i,:])}")
            #print("-" * 20)
        print("*"*20)
    ...
    def plot_likelihoods3(self, envId = 0, policyId = 0):
        # Plot initial and final likelihoods for all Bands
        env = self.envs[envId]
        policy = self.final_policy[envId][policyID]
    ...
    def plotTrueDistributions(self, envId = 0):
        env = self.envs[envId]
        fig, ax = plt.subplots(2,1)
        colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'w']
        # Below is all for NormalGamma model
        for Band in env.Z:
            # Get true distribution
            c = colors[Band.i]
            i = Band.i
            Z_i = Band.Z_i  # Frozen r.v. representing the distribution of E[lam_ij]
            ub = Z_i.ppf(1-.01)+.01 # Upper Bound for linspace
            lb = Z_i.ppf(0+.01)-.01 # Lower Bound for linspace

            n = np.linspace(lb, ub, 500)
            y = Z_i.pdf(n)
            ax[0].plot(n, y, '-', color=c,
                       label=f'Band ({Band.i}) ~ {Z_i.dist.name}(loc={Z_i.kwds["loc"]}, scale = {Z_i.kwds["scale"]})')
            ...
            # binWidth = self.cfg['environment'][envId]["envConfig"]["bandConfig"]["binConfig"]["std"] # Need to implement this better lol
            # ax[1].bar(arms, y_arms, width = binWidth, edgecolor = c, label=f"Empirical Arm Distribution", fill = True, alpha = 0.2 )
            Z_i_emp = Band.Z_i_emp
            y2 = Z_i_emp.pdf(n)
            ax[1].plot(n, y2, '-', color=c,
                       label=f'Band ({Band.i}) ~ {Z_i_emp.dist.name}(loc={"%.2f" % Z_i_emp.kwds["loc"]}, scale = {"%.2f" % Z_i_emp.kwds["scale"]})')



        ax[0].legend(loc='best', frameon=False)
        ax[1].legend(loc='best', frameon=False)
        fig.suptitle(f"Environment: {envId}, Bands: {env.nbBands}, Bins/Band: {env.nbBins}")
        fig.show()
    def plotTrueDistribution(self, envId = 0):
        env = self.envs[envId]
        fig, ax = plt.subplots()
        colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'w']
        # Below is all for NormalGamma model
        ...
        fig.suptitle(f"GENERATIVE/TRUE DISTRIBUTION: Environment: {envId}, Bands: {env.nbBands}, Bins/Band: {env.nbBins}")
        fig.show()

    def plotEmpDistribution(self, envId = 0):
        env = self.envs[envId]
        fig, ax = plt.subplots()
        colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'w']
        # Below is all for NormalGamma model
        for Band in env.Z:
            ...
            i = Band.i
            Z_i_emp = Band.Z_i_emp
            ub = Z_i_emp.ppf(1 - .01) + .01  # Upper Bound for linspace
            lb = Z_i_emp.ppf(0 + .01) - .01  # Lower Bound for linspace

            n = np.linspace(lb, ub, 500)
            y2 = Z_i_emp.pdf(n)
            ax.plot(n, y2, '-', color=c,
                       label=f'Band ({Band.i}) ~ {Z_i_emp.dist.name}(loc={"%.2f" % Z_i_emp.kwds["loc"]}, scale = {"%.2f" % Z_i_emp.kwds["scale"]})')

        ax.legend(loc='best', frameon=False)
        fig.suptitle(f"EMPIRICAL DISTRIBUTION: Environment: {envId}, Bands: {env.nbBands}, Bins/Band: {env.nbBins}")
        fig.show()

    def plotEstDistributions(self, envId = 0, policyId = 0):
        env = self.envs[envId]
        fig, ax = plt.subplots()
        policy = self.final_policy[envId][policyId]
        colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'w']
        for bandInd in range(policy.nbBands):
            c = colors[bandInd]
            X_init = norm(loc=policy.likelihoodParams[bandInd]["_loc"], scale=policy.likelihoodParams[bandInd]["_scale"])
            ub1 = X_init.ppf(1 - .01) + .01  # Upper Bound for linspace
            lb1 = X_init.ppf(0 + .01) - .01  # Lower Bound for linspace
            n1 = np.linspace(lb1, ub1, 500)

            if bandInd == 0:
                ax.plot(n1, X_init.pdf(n1), '--', color='black',
                           label=f'Prior Estimate ~ {X_init.dist.name}(loc={X_init.kwds["loc"]}, scale = {X_init.kwds["scale"]:.3f})')


            X_est = norm(loc=policy.likelihoodParams[bandInd]["loc"], scale=policy.likelihoodParams[bandInd]["scale"])
            ub2 = X_est.ppf(1 - .01) + .01  # Upper Bound for linspace
            lb2 = X_est.ppf(0 + .01) - .01  # Lower Bound for linspace
            n2 = np.linspace(lb2, ub2, 500)
            ax.plot(n2, X_est.pdf(n2), '-', color=c,
                       label=f'Band ({bandInd}) ~ {X_est.dist.name}(loc={X_est.kwds["loc"]:.3f}, scale = {X_est.kwds["scale"]:.3f})')

        ax.legend(loc='topright', frameon=False)
        fig.suptitle(f"Environment: {envId}, Policy: {policyId}, Bands: {env.nbBands}, Bins: {env.nbBins}")
        fig.show()


    def evalEstimates(self, envId = 0, policyId = 0):
        results = {
            "kl:true-emp":{},
            "kl:true-est":{},
            "kl:emp-est": {}
        }
        dists = {} # Gen-Emp, Gen - Est, Emp-Est -> KL Divergence, parameter error
        env = self.envs[envId]
        policy = self.final_policy[envId][policyId]
        kl_true_emp_sum = 0
        kl_true_est_sum = 0
        kl_emp_est_sum = 0
        for bandInd in range(env.nbBands):
            # True distribution

            trueDist = env.Z[bandInd].Z_i
            empDist = env.Z[bandInd].Z_i_emp
            estDist = norm(loc=policy.likelihoodParams[bandInd]["loc"], scale=policy.likelihoodParams[bandInd]["scale"])
            # results[bandInd] = {
            #     "trueDist": trueDist,
            #     "empDist": empDist,
            #     "estDist": estDist,
            #     "kl:true_emp": self.computeEntropy(trueDist, empDist),
            #     "loc:true_emp": np.abs(trueDist.kwds["loc"] - empDist.kwds["loc"])/trueDist.kwds["loc"],
            #     "scale:true_emp": np.abs(trueDist.kwds["scale"] - empDist.kwds["scale"])/trueDist.kwds["scale"],
            #     "kl:true_est": self.computeEntropy(trueDist, estDist),
            #     "loc:true_est": np.abs(trueDist.kwds["loc"] - estDist.kwds["loc"]) / trueDist.kwds["loc"],
            #     "scale:true_est": np.abs(trueDist.kwds["scale"] - estDist.kwds["scale"]) / trueDist.kwds["scale"],
            #     "kl:emp_est": self.computeEntropy(empDist, estDist),
            #     "loc:emp_est": np.abs(empDist.kwds["loc"] - estDist.kwds["loc"])/empDist.kwds["loc"],
            #     "scale:emp_est": np.abs(empDist.kwds["scale"] - estDist.kwds["scale"])/empDist.kwds["scale"],
            # }
            # results[bandInd] = {
            #     "kl:true_emp": float("{:.4f}".format(self.computeEntropy(trueDist, empDist))),
            #     "kl:true_est": float("{:.4f}".format(self.computeEntropy(trueDist, estDist))),
            #     "kl:emp_est": float("{:.4f}".format(self.computeEntropy(empDist, estDist))),
            # }
            results["kl:true-emp"][bandInd] = float("{:.4f}".format(self.computeEntropy(trueDist, empDist)))
            results["kl:true-est"][bandInd] = float("{:.4f}".format(self.computeEntropy(trueDist, estDist)))
            results["kl:emp-est"][bandInd] = float("{:.4f}".format(self.computeEntropy(empDist, estDist)))

            kl_true_emp_sum +=results["kl:true-emp"][bandInd]
            kl_true_est_sum +=results["kl:true-est"][bandInd]
            kl_emp_est_sum +=results["kl:emp-est"][bandInd]
            dists[bandInd] = {
                "trueDist": trueDist,
                "empDist": empDist,
                "estDist": estDist,}
        results["kl:true-emp"]["sum"] = kl_true_emp_sum
        results["kl:true-est"]["sum"] = kl_true_est_sum
        results["kl:emp-est"]["sum"] = kl_emp_est_sum
        print(f"Printing KL stats for Environment {envId}, Policy {policyId}")
    ...
    def computeEntropy(self, dist1, dist2):
        # Get Bounds of distributions
        ub1 = dist1.ppf(1-.001)
        ub2 = dist2.ppf(1-.001)
        lb1 = dist1.ppf(.001)
        lb2 = dist1.ppf(.001)
        ...
        pdf1 = dist1.pdf(x)
        pdf2 = dist2.pdf(x)
        kl = entropy(pdf1, pdf2, base = 2)
        return kl


    def plot_likelihoods2(self, envId = 0, policyId = 0, arms = False):
        env = self.envs[envId]
        policy = self.final_policy[envId][policyId]
        fig, ax = plt.subplots(2, 1, sharey=True, sharex=True)
        ...
        if issubclass(policy.posteriorDist, Uniform):
            for Band in env.X:
                ...
                ax[0].bar(arms, y_arms, width = binWidth, edgecolor = c, label=f"Empirical Arm Distribution", fill = True, alpha = 0.2 )
        ...
        if issubclass(policy.posteriorDist, NormalGamma):
            for Band in env.Z:
                # Get true distribution
                c = colors[Band.i]
                i = Band.i
                ...
                X_init = norm(loc = policy.likelihoodParams[i]["_loc"], scale = policy.likelihoodParams[i]["_scale"])
                ub1 = X_init.ppf(1-.01) + .01  # Upper Bound for linspace
                lb1 = X_init.ppf(0+.01) - .01  # Lower Bound for linspace
                n1 = np.linspace(lb1, ub1, 500)
                #c2 = colors[Band.i * 2]
                ...
                X_est = norm(loc = policy.likelihoodParams[i]["loc"], scale = policy.likelihoodParams[i]["scale"])
                ub2 = X_est.ppf(1-.01) + .01  # Upper Bound for linspace
                lb2 = X_est.ppf(0+.01) - .01  # Lower Bound for linspace
                n2 = np.linspace(lb2, ub2, 500)
                ax[1].plot(n2, X_est.pdf(n2), '-', color=c2,
    ...
    def plot_likelihoods3(self, envId =0, policyID=0):
        # True distributions from environment
        env =self.envs[envId]
        policy = self.final_policy[envId][policyID]
        fig, ax = plt.subplots(2,1, sharey= True, sharex= True)
        #plt.rcParams.update({"text.usetex": True,})
        colors = palette(env.nbBands*2)
        markers = makemarkers(self.nbPolicies)
        for Band in env.X:
            # Get true distribution
            c = colors[Band.i]
            i = Band.i
            X_i = Band.X_i  # Frozen r.v. representing the distribution of E[lam_ij]
            ub = X_i.ppf(.999) # Upper Bound for linspace
            lb = X_i.ppf(.001) # Lower Bound for linspace

            n = np.linspace(lb, ub, 100)
            ax[0].plot(n, X_i.pdf(n), '--', color = c, label = f'Band ({Band.i}) ~ N({X_i.kwds["loc"]},{X_i.kwds["scale"]**2})')
            #ax[1].plot(n, X_i.pdf(n),'--' , color = c, label = f'Band ({Band.i}) ~ N({X_i.kwds["loc"]},{X_i.kwds["scale"]**2})', alpha = 0.5)
            arms = Band.lam
            ax[0].hist(arms, color = c, density = True, histtype = 'bar', label = f"Empirical Arm Distribution", alpha = 0.4)

            #get initial posterior paramers
            eta0, lam0, alpha0, beta0 = policy.posteriorBand[i].getInitParams()
            Mu0 = eta0
            Std0 = np.sqrt(beta0/alpha0)
            Y_i = norm(loc = Mu0, scale = Std0)
            c2 = colors[Band.i*2]
            ax[1].plot(n, Y_i.pdf(n), '.', color = c2, label = f'Band ({Band.i}) init est ~ N({Y_i.kwds["loc"]},{Y_i.kwds["scale"]**2}) ', alpha = 0.5)

            # get final estimated distribution
            MuF = policy.likelihoodParams[i][0]
            StdF = np.sqrt(1/policy.likelihoodParams[i][1])
            B_i = norm(loc=MuF, scale=StdF)
            ax[1].plot(n, B_i.pdf(n), color=c2,
                       label=f'Band ({Band.i}) final est ~ N({B_i.kwds["loc"]:.3f},{B_i.kwds["scale"] ** 2:.3f}) ', alpha=1)
    ...
    def printHMABResults(self, envId, policyId):
        env = self.envs[envId]
        env.printEnvInfo()
    ...
    def plotRewards(self, envId, policyId):
        fig, ax = plt.subplots()
        for eId in envId:
            for pId in policyId:
                policy = self.final_policy[eId][pId]
                env = self.envs[eId]
                if isinstance(policy, float) or isinstance(env, float):
                    continue
                elif not (policy.isHMAB == env.isHMAB):
                    continue
                rewards = self.final_result[eId][pId].rewards
                ax.plot(rewards, '-', label = f"EnvId = {eId}, PolId = {pId}")
        ax.legend(loc='best', frameon=False)
        fig.show()

    def printSummary(self,envId = 0, policyId =0, topk = 3, algoParams = True):
        env = self.envs[envId]
        pol = self.final_policy[envId][policyId]
        BB_env = np.array(env.BBsorted[:,:topk])
        BB_pol = np.array(pol.BBsorted[:,:topk])
        arm_env = env.bestArms[:topk]
        arm_pol = pol.bestArms[:topk]
        nl = '\n'
        print("*"*20)
        print("Printing Summary")
        if algoParams:
            pol.printAlgoParams()
        print(f"(A) ENVIRONMENT BAND BEST FIT DISTRIBUTIONS")
        env_p_vec = env.printBestFits(percentile=True)
        print(f"(B) POLICY ESTIMATED BAND DISTRIBUTIONS")
        pol_p_vec = pol.printBandEsts(percentile= True)
        print(f"(C) ENVIRONMENT - BEST {topk} BINS")
        env_mus = env.printTopKBins(topk)
        print(f"(D) POLICY - BEST {topk} BINS")
        pol_mus = pol.printTopKBins(topk, samples = True)
        print(f"(E) POLICY ESTIMATES OF ENV's BEST {topk} BINS")
        pol.printBins(arm_env, samples = True)
        print(f"(F) ACTUAL DIST OF POLICY's BEST {topk} BINS")
        pol_mus = env.printBins(arm_pol)
        print(f"(G) ERROR ANALYSIS")
        print(f"Actual Goal Coefficient of Variation {np.max(env_p_vec)}")
        print(f"Estimated Goal Coefficient of Variation {np.max(pol_p_vec)}")
        print(f"Chosen Bin's Actual Percentile {round(env.computePercentile(np.argmax(pol_p_vec),arm_pol[0]),3)}")
        print(f"Chosen Bin Expected Value = {round(pol_mus[0],3)}")
        success = pol_mus[0] >=np.max(env_p_vec)
        print(f"Success: {success}")
        print(f"Chosen Bin {tuple(BB_pol[:,0])} - Best Bin {tuple(BB_env[:,0])}  = {round(pol_mus[0]-env_mus[0],4)}")

        return 0
### Nothing changed yet, eventually want to support multisampling

def delayed_HMAB_play(env, policy, horizon,
    ...
    if env.isHMAB:
        ...
        result = Result(env.nbArms, horizon, indexes_bestarm=indexes_bestarm, means=means, isHMAB=env.isHMAB,
                        **result_args)  # One Result object, for every policy
    else:
        result = Result(env.nbArms, horizon, indexes_bestarm=indexes_bestarm, means=means, isHMAB=False)
    ...
    while t < horizon:
        ...
        stop = policy.checkStop()
        if stop:
            break
        if policy.nbDraws > 1:
            #1. A players policy chooses an arm

            choice = policy.choice()

            #2. policy.nbDraw rewards are drawn from the arm at this time
            reward = env.draw(choice, t, policy.nbDraws)

            # 3. The policy sees the reward(s)
            policy.getReward(choice,reward)

            #Result has been changed to factor in multiple rewards drawn
    ...
    # Modify results to get rid of unused timesteps
    result.updateT(t)
    policy.getStats()
...
def delayed_play(env, policy, horizon,
    ...
    if env.isHMAB:
        ...
        result = Result(env.nbArms, horizon, indexes_bestarm=indexes_bestarm, means=means, isHMAB=env.isHMAB, **result_args)  # One Result object, for every policy
    else:
        result = Result(env.nbArms, horizon, indexes_bestarm=indexes_bestarm, means=means, isHMAB=False)
    # FIXME Monkey patching policy.detect_change() to store number of detections, see https://stackoverflow.com/a/42657312/
    if hasattr(policy, 'detect_change'):
