...
class Evaluator(object):
    ...
    def plotRegrets(self, envIds=0,
        ...
        for envId in envIds:
            ...
            #     print("\nThis MAB problem has: \n - a [Lai & Robbins] complexity constant C(mu) = {:.3g} for 1-player problem... \n - a Optimal ArmBelief Identification factor H_OI(mu) = {:.2%} ...".format(lowerbound, self.envs[envId].hoifactor()))  # DEBUG
    ...
    def plotArmPulls(self, envId, policyId):
        ...
        if envBestArm == polBestArm:
            bar[envBestArm].set_label("Env. & Policy Best ArmBelief")
        else:
            bar[envBestArm].set_label("Env. (True) Best ArmBelief")
            bar[polBestArm].set_label("Policy (Est) Best ArmBelief")
        ax.set_ylabel("Pulls")
        ax.set_xlabel("ArmBelief")
        plt.xticks(x)
        ax.set_title(f'Total ArmBelief Pulls for EnvId ({envId}) & Policy ({policyId}) ')
        ax.legend()
    ...
    def plotTrueDistributions(self, envId = 0):
        ...
        for Band in env.Z:
            ...
            # ax[1].bar(arms, y_arms, width = binWidth, edgecolor = c, label=f"Empirical ArmBelief Distribution", fill = True, alpha = 0.2 )
    ...
    def plot_likelihoods3(self, envId =0, policyID=0):
        ...
        for Band in env.X:
            ...
            arms = Band.lam
            ax[0].hist(arms, color = c, density = True, histtype = 'bar', label = f"Empirical ArmBelief Distribution", alpha = 0.4)
...
def delayed_HMAB_play(env, policy, horizon,
    ...
    means = env.means  # ArmBelief means
