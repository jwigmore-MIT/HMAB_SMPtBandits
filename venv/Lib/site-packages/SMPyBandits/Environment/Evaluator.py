...
try:
    ...
    from ..Policies.Posterior.Uniform import Uniform
    from ..Policies.Posterior.NormalGamma import NormalGamma
    from ..Policies.Posterior.NormalMLE import NormalMLE
except ImportError:
    # Local imports, libraries
    from usejoblib import USE_JOBLIB, Parallel, delayed
    from usetqdm import USE_TQDM, tqdm
...
class Evaluator(object):
    ...
    def startOneEnv(self, envId, env):
        ...
        for policyId, policy in enumerate(self.policies):
            ...
            if self.useJoblib:
                ...
                for repeatId in tqdm(range(self.repetitions), desc="Repeat"):
                    r,p = delayed_play(env, policy, self.horizon, random_shuffle=self.random_shuffle, random_invert=self.random_invert, nb_break_points=self.nb_break_points, allrewards=allrewards, repeatId=repeatId, useJoblib=self.useJoblib)
    ...
    # --- Get data
    def getBestArm(self, envId, policyId, verbose = True):
        ...
        if env.isHMAB and env.isHMAB:
            envBestArm = self.envs[envId].bestArm
            envBestArmVal = self.envs[envId].BestBandBinValue
            polBestArmVal = np.max(self.final_policy[envId][policyId].lam_est)
            polBestArm = np.argmax(self.final_policy[envId][policyId].lam_est)
        if not env.isHMAB and not pol.isHMAB:
            ...
            pass
        return (envBestArm, envBestArmVal, polBestArm, polBestArmVal)
        #pass
        if verbose:
            print("*" * 20)
    ...
    def plotArmPulls(self, envId, policyId):
        ...
        (envBestArm, envBestArmVal, polBestArm, polBestArmVal) = self.getBestArm(envId, policyId, verbose= False)
