...
class HMABenv(object):
    ...
    def __init__(self, envConfig):
        ...
        :param envConfig: <dict> contains parameters to set up the MAB environment
                ...
                "bandConfig": <dict> contains parameters to set up each Band object in the environment
        ...
        if isinstance(envConfig, dict):
            ...
            # self.bandDistribution = envConfig["bandDistribution"]       # Mean reward Distribution for each Band i.e. dist of X_i e.g. "Gaussian"
            # self.binDistribution = envConfig["binDistribution"]
            self.X = list()                 # List of X_i's
            for n in range(self.nbBands):
                # params = {
                #     "i": n,
                #     "nbBins": self.nbBins,
                #     'bandDistribution': self.bandDistribution,
                #     'bandParams': envConfig["bandParams"][n],
                #     'binDistribution': self.binDistribution
                # }
                self.X.append(self.Band(envConfig,n))
                self.arms.extend(self.X[-1].arms)
        # Figure out mean computations plus storing arm values in Environment
        # Then go through delayed_play and make sure everything checks out
    ...
    class Band:
        ...
        def __init__(self, envConfig, n):
            ...
            self.bandDistribution = bandConfig["bandDistribution"]  # Distribution of X_i i.e. mean reward distribution
            self.binDistribution = bandConfig["binDistribution"]
            if self.bandDistribution == "Gaussian":
                self.Mu = bandParams["bandParams"][0]  # Band mean
                self.Std = bandParams["bandParams"][1]   # Band standard deviation
                self.X_i = norm(loc = self.Mu, scale = self.Std) # Distribution of the Band (scipy rv object)
                self.arms = list()
                self.lam = self.X_i.rvs(self.nbBins) # Create bin means by sampling X_i
                self.bin_std = 1 # set bin standard deviation
                self.loc = self.Mu
                self.scale = self.Std
                for l in self.lam:
                ...
                self.X_i = uniform(loc = self.min, scale = self.scale) #Distribution of Band
                self.arms = list()
                self.lam = self.X_i.rvs(self.nbBins) # Sample of means
            ...
            # Now we need to create M arms with arm_type Gaussian and means sampled from X_i
    ...
    def draw(self, choice, t=1):
        ...
        Band = choice[0]
        Bin = choice[1]
        reward = self.X[Band].arms[Bin].draw(t)
        #print(f"Drawing from (Band, Bin): {choice} which has a rewards ~ N({self.X[Band].arms[Bin].mean:0.3f},{self.X[Band].arms[Bin].sigma**2})")
