...
class HMABenv(object):
    ...
    def __init__(self, envConfig):
        ...
        if envConfig["verbose"]:
            ...
            for B in range(self.nbBands):
                ...
                print(f"({B})|ArmBelief Distributions = [{Band.arms}]")
                print(f"({B})|Best ArmBelief = [{Band.bestArm}] with expected reward  = {Band.binMeans[Band.bestArm]:0.4f}")
        ...
        if envConfig["verbose"]:
            ...
            print(f"Best (Band,Bin) [ArmBelief] = {self.BBsorted[:,0]} [{self.bestArms[0]}]  w/ expected reward = {self.maxArm}")
            print("*" * 20)
    ...
    def computePercentile(self, Band,ArmBelief):
        # ArmBelief is non BB
    ...
    class Band:
        ...
        def __init__(self, bandConfig, n):
            ...
            self.bestArm = np.argmax(self.binMeans)  # ArmBelief with the highest expected reward within the Band
