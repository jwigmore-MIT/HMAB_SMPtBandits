...
class MAB(object):
    ...
    def __init__(self, configuration):
        ...
        self.bestArm = np.argmax(self.means)  #: ArgMax mean of arms
        print(" - with 'argmaxArm' =", self.bestArm)  # DEBUG
        self.minArm = np.min(self.means)  #: Min mean of arms
        print(" - with 'minArm' =", self.minArm)  # DEBUG
        # Print lower bound and HOI factor
        print("\nThis MAB problem has: \n - a [Lai & Robbins] complexity constant C(mu) = {:.3g} ... \n - a Optimal ArmBelief Identification factor H_OI(mu) = {:.2%} ...".format(self.lowerbound(), self.hoifactor()))  # DEBUG
    ...
    def hoifactor(self):
        """ Compute the HOI factor H_OI(mu), the Optimal ArmBelief Identification (OI) factor, for this MAB problem (complexity). Cf. (3.3) in Navikkumar MODI's thesis, "Machine Learning and Statistical Decision Making for Green Radio" (2017)."""
...
class ChangingAtEachRepMAB(MAB):
    ...
    def hoifactor(self):
        """ Compute the HOI factor H_OI(mu), the Optimal ArmBelief Identification (OI) factor, for this MAB problem (complexity). Cf. (3.3) in Navikkumar MODI's thesis, "Machine Learning and Statistical Decision Making for Green Radio" (2017) (averaged on all the draws of new means)."""
...
class PieceWiseStationaryMAB(MAB):
    ...
    def plotHistoryOfMeans(self, horizon=None, savefig=None, forceTo01=False, showplot=True, pickleit=False):
        ...
        for armId in range(self.nbArms):
            ...
            plt.plot(meanOfThisArm, color=colors[armId], marker=markers[armId], markevery=(armId / 50., 0.1), label='ArmBelief #{}'.format(armId), lw=4, alpha=0.9)
        legend()
        ymin, ymax = plt.ylim()
        if forceTo01:
