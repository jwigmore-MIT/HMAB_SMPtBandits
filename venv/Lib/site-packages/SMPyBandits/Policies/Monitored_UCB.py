...
class Monitored_IndexPolicy(BaseWrapperPolicy):
    ...
    def __init__(self, nbArms,
        ...
        self.last_update_time_tau = 0  #: Keep in memory the last time a change was detected, ie, the variable :math:`\tau_i` in the algorithm.
    ...
    def getReward(self, arm, reward):
        ...
        if self.detect_change(arm):
            # print("For a player {} a change was detected at time {} for arm {}, after {} pulls of that arm (giving mean reward = {:.3g}). Last restart on that arm was at tau_i = {}".format(self, self.t, arm, self.last_pulls[arm], np.sum(self.last_w_rewards[arm]) / self.last_pulls[arm], self.last_restart_times[arm]))  # DEBUG
