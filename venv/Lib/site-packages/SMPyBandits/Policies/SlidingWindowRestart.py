# -*- coding: utf-8 -*-
r""" An experimental policy, using a sliding window (of for instance :math:`\tau_i=100` *draws* of each arm), and reset the algorithm as soon as the small empirical average is too far away from the long history empirical average (or just restart for one arm, if possible).
...
- It runs on top of a simple policy, e.g., :class:`UCB`, and :func:`SlidingWindowRestart` is a generic policy using any simple policy with this "sliding window" trick:
    ...
    >>> policy = SlidingWindowRestart(nbArms, UCB, tau_i=100, threshold=0.1)
    >>> # use policy as usual, with policy.startGame(), r = policy.choice(), policy.getReward(arm, r)

- It uses an additional :math:`\mathcal{O}(\tau_i)` memory but do not cost anything else in terms of time complexity (the average is done with a sliding window, and costs :math:`\mathcal{O}(1)` at every time step).
...
class SlidingWindowRestart(BaseWrapperPolicy):
    ...
    def __init__(self, nbArms, policy=DefaultPolicy,
        ...
        # New parameters
        assert 1 <= tau, "Error: parameter 'tau_i' for class SlidingWindowRestart has to be >= 1, but was {}.".format(tau)  # DEBUG
        ...
        # Internal memory
        self.last_rewards = np.zeros((nbArms, self._tau))  #: Keep in memory all the rewards obtained in the last :math:`\tau_i` steps.
...
class SWR_UCB(UCB):
    ...
    def __init__(self, nbArms, tau=TAU, threshold=THRESHOLD, full_restart_when_refresh=FULL_RESTART_WHEN_REFRESH, *args, **kwargs):
        ...
        # New parameters
        assert 1 <= tau, "Error: parameter 'tau_i' for class SWR_UCB has to be >= 1, but was {}.".format(tau)  # DEBUG
        ...
        self.last_rewards = np.zeros((nbArms, tau))  #: Keep in memory all the rewards obtained in the last :math:`\tau_i` steps.
        self.last_pulls = np.zeros(nbArms, dtype=int)  #: Keep in memory the times where each arm was last seen. Start with -1 (never seen)
...
class SWR_UCBalpha(UCBalpha):
    ...
    def __init__(self, nbArms, tau=TAU, threshold=THRESHOLD, full_restart_when_refresh=FULL_RESTART_WHEN_REFRESH, alpha=ALPHA, *args, **kwargs):
        ...
        # New parameters
        assert 1 <= tau, "Error: parameter 'tau_i' for class SWR_UCBalpha has to be >= 1, but was {}.".format(tau)  # DEBUG
        self.tau = int(tau)  #: Size of the sliding window.
...
class SWR_klUCB(klUCB):
    ...
    def __init__(self, nbArms, tau=TAU, threshold=THRESHOLD, full_restart_when_refresh=FULL_RESTART_WHEN_REFRESH, tolerance=1e-4, klucb=klucbBern, c=c, *args, **kwargs):
        ...
        # New parameters
        assert 1 <= tau, "Error: parameter 'tau_i' for class SWR_klUCB has to be >= 1, but was {}.".format(tau)  # DEBUG
        self.tau = int(tau)  #: Size of the sliding window.
