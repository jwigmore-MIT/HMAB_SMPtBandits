...
class HMABPolicy(BasePolicy):
    def __init__(self, nbBands, nbBins, BayesSetup = None, verbose = True):
        """
        ...
        ### DEPRECATED - ALWAYS USE getBB() to get the same information, no need to store twice
        # ex: self.pullsBB[B,b] = self.pulls(self.getArm(Band, Bin))
        ...
        # self.binMeanEstBB = np.zeros((nbBands, nbBins))   #: Estimated expected reward for the bin based on observations
    ...
    class Band:
        def __init__(self, Policy, index, nbBins, distribution, initParams):
            self.Policy = Policy
            self.nbBins = nbBins
            self.index = index
    ...
    class Bin:
        def __init__(self, Band, bindex, Distribution, initParams):
            self.Band = Band
            self.bindex = bindex
