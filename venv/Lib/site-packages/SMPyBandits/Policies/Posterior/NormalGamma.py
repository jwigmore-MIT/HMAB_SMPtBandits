...
class NormalGamma(Distribution):
    ...
    def __init__(self, omega, zeta, alpha, beta):
        r"""Create a posterior assuming the prior is : N(omega, tau^(-1))..
        """
    ...
    def reset(self, *args):
        r""" Reset the for parameters :omega and tau, as when creating a new NormalGamma posterior."""
        if not args:
    ...
    def sample(self):
        ...
        # Sample first from gamma(alpha, beta) to get T (tau)
    ...
    def plotDist(self, ax):
        ...
        #         # ax[0].hist(arms, bins = env.nbArms*50, color=c, density=False, histtype='bar', label=f"Empirical Arm Distribution", alpha=1)
        #         binWidth = self.cfg['environment'][envId]["envConfig"]["bandConfig"]["binConfig"][
        #             "scale"]  # Need to implement this better lol
        #         ax[0].bar(arms, y_arms, width=binWidth, edgecolor=c, label=f"Empirical Arm Distribution", fill=True,
        #                   alpha=0.2)
        #
