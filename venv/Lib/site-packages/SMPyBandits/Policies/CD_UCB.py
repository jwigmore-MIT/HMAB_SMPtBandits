...
class CD_IndexPolicy(BaseWrapperPolicy):
    ...
    def getReward(self, arm, reward):
        ...
        .. warning:: If the :math:`detect_change` method also returns an estimate of the position of the change-point, :math:`\hat{tau_i}`, then it is used to reset the memory of the changing arm and keep the observations from :math:`\hat{tau_i}+1`.
        """
        ...
        if position is None:
            # print("For a player {} a change was detected at time {} for arm {}, after {} pulls of that arm (giving mean reward = {:.3g}). Last restart on that arm was at tau_i = {}".format(self, self.t, arm, self.last_pulls[arm], np.sum(self.all_rewards[arm]) / self.last_pulls[arm], self.last_restart_times[arm]))  # DEBUG
