...
class HMABAlgo1(HMABPolicy):
    ...
    def __init__(self,nbBands, nbBins, BayesSetup = None, verbose = True, AlgoParams = None):
        ...
        self.percentile = AlgoParams["percentile"]
          # Number of "Draws" or samples from an arm every time it is chosen - may change between phases
    ...
    def initialize_distributions(self):
        ...
        for B in range(self.nbBands):
            ...
            self.Bands[B] = self.Band(self, B, self.nbBins, self.DistributionSetup["BandDist"], self.DistributionSetup["InitBandParams"])
            for b in range(self.nbBins):
                self.Bands[B].Bins[b] = self.Bin(self.Bands[B], b, self.DistributionSetup["BinDist"], self.DistributionSetup["InitBinParams"])
    ...
    def choice(self):
        ...
        if self.Phase ==1: ## We are in phase1
            ...
            if muMax >= p_max:
                BinChoice = bestBin
            elif Band.BinSampled.__len__() >= self.nbBins:
                BinChoice = bestBin
            else:
